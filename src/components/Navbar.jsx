...
export default function Navbar() {
  ...
  // Dropdown Tools Kategori dengan Flag `isAvailable`
  const toolCategories = [
    {
      ...
      icon: Calculator,
      color: "text-cyan-400",
      isAvailable: true, // AKTIF BISA DIPENCET
    },
    {
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            >
              <Link
                href="/tools"
                className={`relative flex items-center gap-1 px-4 py-2 text-[11px] font-black uppercase tracking-widest transition-all duration-300 rounded-lg ${
              ...
              {/* Dropdown Content */}
              {toolsDropdownOpen && (
                <div className="absolute top-full -left-4 w-72 bg-[#0d0e12]/95 border border-white/10 rounded-2xl p-3 shadow-2xl backdrop-blur-2xl space-y-1 animate-in fade-in slide-in-from-top-2 duration-200">
                  ...
                  {toolCategories.map((cat, idx) => {
                    ...
                    if (cat.isAvailable) {
                      return (
                        ...
                        >
                          <div className={`p-2 rounded-lg bg-white/5 border border-white/10 group-hover:border-cyan-500/30 ${cat.color}`}>
                    ...
                    {/* Non-active / Coming Soon Item (Tidak bisa dipencet) */}
                    return (
                      <div
                        key={idx}
                  ...
                  {/* Explore All Tools (BISA DIPENCET) */}
                  <div className="pt-2 border-t border-white/5 mt-1">
                    <Link
                      href="/tools"
                      onClick={() => setToolsDropdownOpen(false)}
                      className="flex items-center justify-between p-2.5 rounded-xl bg-cyan-500/10 text-cyan-400 text-xs font-bold hover:bg-cyan-500/20 transition-colors"
                    >
            ...
            <Link
              href="/service"
              className={`relative flex items-center gap-2 px-4 py-2 text-[11px] font-black uppercase tracking-widest transition-all duration-300 rounded-lg ${
        ...
        <div className="bg-[#0a0a0c]/95 backdrop-blur-xl border-t border-white/[0.06] px-6 py-6 space-y-4">
              ...
              if (cat.isAvailable) {
                return (
                  <Link
                    ...
                    href={cat.href}
                    onClick={() => setMenuOpen(false)}
                    className="block text-xs text-gray-300 hover:text-white pl-3 py-1 border-l border-white/10"
                  >
                    {cat.label}
                  </Link>
                );
          ...
          <div className="pt-2">
            <Link
              href="/tools"
              onClick={() => setMenuOpen(false)}
              className="block w-full text-center py-3 rounded-xl bg-cyan-500 text-black font-black text-xs uppercase tracking-wider"
            >
              Launch Tools
            </Link>
          </div>
